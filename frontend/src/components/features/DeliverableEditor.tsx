...
import {
  listDeliverablesRouteApiV1WorkspacesWorkspaceIdProjectsProjectIdOppmDeliverablesGet,
  createDeliverableRouteApiV1WorkspacesWorkspaceIdProjectsProjectIdOppmDeliverablesPost,
  updateDeliverableRouteApiV1WorkspacesWorkspaceIdOppmDeliverablesItemIdPut,
  deleteDeliverableRouteApiV1WorkspacesWorkspaceIdOppmDeliverablesItemIdDelete,
...
export function DeliverableEditor({ projectId }: DeliverableEditorProps) {
  const queryClient = useQueryClient()
  const ws = useWorkspaceStore((s) => s.currentWorkspace)

  const { data: items, isLoading } = useQuery({
    queryKey: ['oppm-deliverables', projectId, ws?.id],
    queryFn: async () => {
      if (!ws) return []
      const res = await listDeliverablesRouteApiV1WorkspacesWorkspaceIdProjectsProjectIdOppmDeliverablesGet({
        client: workspaceClient,
        path: { workspace_id: ws.id, project_id: projectId },
      })
      if (res.error) {
        throw new Error((res.error as { detail?: string })?.detail || 'Failed to load deliverables')
      }
  ...
  const create = useMutation({
    mutationFn: async (itemNumber: number) => {
      ...
      const res = await createDeliverableRouteApiV1WorkspacesWorkspaceIdProjectsProjectIdOppmDeliverablesPost({
        client: workspaceClient,
        path: { workspace_id: ws.id, project_id: projectId },
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['oppm-deliverables'] })
  ...
  const update = useMutation({
    mutationFn: async ({ id, description }: { id: string; description: string }) => {
      if (!ws) throw new Error('No workspace')
      const res = await updateDeliverableRouteApiV1WorkspacesWorkspaceIdOppmDeliverablesItemIdPut({
        client: workspaceClient,
  ...
  const remove = useMutation({
    mutationFn: async (itemId: string) => {
      if (!ws) throw new Error('No workspace')
      const res = await deleteDeliverableRouteApiV1WorkspacesWorkspaceIdOppmDeliverablesItemIdDelete({
        client: workspaceClient,
  ...
  const byNumber = new Map<number, Deliverable>()
  ...
  return (
    <div className="space-y-2">
      <div className="text-xs font-medium text-gray-500">Deliverables (1-4)</div>
      {isLoading ? (
