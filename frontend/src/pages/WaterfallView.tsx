  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          {/* Pipeline bar */}
          <div className="flex items-center gap-1 rounded-xl border border-border bg-white p-3 shadow-sm">
              ...
              return (
                <div key={phase.id} className="flex items-center flex-1">
                  <button
                    ...
                    className={cn(
                      'flex flex-1 items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition-colors',
          ...
          <div className="space-y-3">
            {phaseList.map((phase) => {
              ...
              return (
                <div key={phase.id} className="rounded-xl border border-border bg-white shadow-sm overflow-hidden">
                  <button
                  ...
                  {isExpanded && (
                    <div className="border-t border-border p-4 space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {canApprove && (
                          <button
                            ...
                            className="ml-2 flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 transition-colors disabled:opacity-50"
                          >
                      ...
                      {/* Phase notes */}
                      {phase.notes && (
                        <div className="rounded-lg bg-muted/50 p-3 text-sm text-text-secondary">{phase.notes}</div>
                      )}
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-lg border border-border px-2.5 py-1 text-xs hover:bg-muted transition-colors"
                          >
                              ...
                              <div key={doc.id} className="flex items-center justify-between rounded-lg border border-border p-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    <span className="ml-2 rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-text-secondary">
      ...
      {showCreateDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <form onSubmit={(e) => handleCreateDoc(e, showCreateDoc)} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input name="title" required maxLength={300} placeholder="Document title" className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium">Type</label>
                <select name="document_type" defaultValue="general" className="w-full rounded-lg border border-border px-3 py-2 text-sm">
              ...
              <div>
                ...
                <textarea name="content" rows={6} placeholder="Markdown or plain text…" className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setShowCreateDoc(null)} className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-muted">Cancel</button>
              <button type="submit" disabled={createDocMut.isPending} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-50">
