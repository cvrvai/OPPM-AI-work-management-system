...
|---|---|---|
| **Design & develop full-stack web applications** | React frontend + FastAPI backend with microservices | `/frontend/src/` and `/services/workspace/` |
| **Clean, testable, well-documented code** | Type hints, comprehensive docstrings, 20+ tests | `/services/workspace/domains/`, test files |
...
| **RESTful and GraphQL APIs** | Full REST v1 coverage + GraphQL ready | `/services/workspace/domains/`, intelligence service |
...
| **SOLID principles & clean architecture** | Repository pattern, dependency injection, separation of concerns | `/services/workspace/` DDD domain structure |
...
│  ├── Workspace Service (Auth, Projects, Tasks, Workspaces)         │
│  ├── Intelligence Service (GraphQL, LLM, RAG)                            │
│  ├── Integrations Service (GitHub integration)                          │
│  └── Automation Service (Model Context Protocol)                      │
├─────────────────────────────────────────────────────────────────┤
│ Persistence Layer                                               │
...
**Key Pattern Files:**
- `services/workspace/domains/*/*.py` - Endpoint definitions
- `services/workspace/domains/*/*.py` - Business logic
- `services/workspace/domains/*/*.py` - Data access
...
# - Intelligence Service: http://localhost:8001
...
**Implementation:**
- DDD domain architecture (domains/{domain}/router.py, service.py, repository.py, schemas.py)
- Type hints on all functions
- Comprehensive docstrings
- Error handling at each layer
- Dependency injection via FastAPI `Depends()`

**Files:** `services/workspace/domains/`, `services/workspace/domains/`, `services/workspace/domains/`
...
**GraphQL-Ready:**
- Intelligence service prepared for GraphQL queries
- Schema design documented

**Files:** `services/workspace/domains/`, `docs/API-REFERENCE.md`
...
**Files:** `shared/auth.py`, `services/workspace/domains/auth/middleware.py`
...
**Answer:** "This is a multi-tenant SaaS platform using microservices. The frontend is React 19 with TypeScript, communicating via REST APIs to four FastAPI backend services through an Nginx gateway. The Workspace service handles auth, projects, and tasks using a DDD domain architecture. Everything runs on Docker for consistency."
...
### Backend Excellence
- `services/workspace/domains/` - API endpoint design
- `services/workspace/domains/` - Business logic separation
- `services/workspace/domains/` - Data access abstraction
...
1. **Add GraphQL implementation** (Intelligence service)
