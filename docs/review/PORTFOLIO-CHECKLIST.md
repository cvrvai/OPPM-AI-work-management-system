...
| **GraphQL API design** | ✅ READY | Intelligence service prepared, schema designed |
...
- **DDD Domain Architecture:** See `/services/workspace/` structure
...
**Key files to review:**
```
services/workspace/domains/project/router.py       # Router layer
services/workspace/domains/project/service.py       # Service layer
services/workspace/domains/project/repository.py      # Repository layer
...
**Show this:** Look at error handling in `services/workspace/domains/auth/router.py`
...
### ✅ GraphQL Ready
- Intelligence service has GraphQL schema prepared
...
```bash
# Show type-safe code
cd services/workspace && grep -A 5 "def create_project" domains/project/service.py

# Show clean architecture layers
ls -la services/workspace/domains/

# Show error handling
grep -B 2 -A 2 "HTTPException" services/workspace/domains/auth/router.py
```
...
grep -r "def.*_route" services/workspace/domains/*/router.py | head -10

# Show response schemas
head -30 services/workspace/domains/project/schemas.py
```
...
**Answer:** "This is a multi-tenant SaaS platform using microservices. Frontend is React 19 with TypeScript, connected via REST APIs to four FastAPI services through Nginx gateway. The Workspace service handles auth and projects using DDD domain architecture: each domain (auth, workspace, project, task, oppm, notification, member, dashboard, agile, waterfall) has its own router, service, repository, and schemas co-located. We use SQLAlchemy async ORM for the database. Everything runs on Docker for consistency. Why microservices? Independent scaling and deployment. Why DDD? Testability, maintainability, and clear domain boundaries."
