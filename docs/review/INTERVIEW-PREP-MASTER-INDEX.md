...
- `services/workspace/domains/project/router.py` - API endpoint design
- `services/workspace/domains/project/service.py` - Business logic layer
- `services/workspace/domains/project/repository.py` - Data access layer
- `shared/models/project.py` - ORM model definition
- `shared/auth.py` - JWT authentication implementation
- `services/workspace/domains/` - Pydantic validation schemas
...
**Architecture (3 min):**
> "The system has four microservices running in Docker: Workspace handles auth and projects, Intelligence service for LLM features, Integrations service for GitHub integration, and Automation service for protocol support. Everything uses a DDD domain architecture with domains containing router, service, repository, and schemas. This enables testability, scalability, and maintainability."
...
**API Design (2 min):**
> "REST API following the pattern /api/v1/workspaces/{id}/resource. Consistent pagination, proper HTTP status codes, input validation with Pydantic. GraphQL is prepared in the Intelligence service."
