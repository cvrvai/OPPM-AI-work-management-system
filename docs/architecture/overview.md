...
- [ER-DIAGRAM.md](database/ER-DIAGRAM.md)
- [FLOWCHARTS.md](architecture/flowcharts.md)
- [miro/flowcharts.md](miro/flowcharts.md) ← Miro-ready flowcharts
- [miro/er-diagram.md](miro/er-diagram.md) ← Miro-ready ER diagram
- [miro/api-pipeline.md](miro/api-pipeline.md) ← Miro-ready API interactions
- [miro/rag-system.md](miro/rag-system.md) ← Miro-ready RAG pipeline diagrams
- [miro/ai-tool-calling.md](miro/ai-tool-calling.md) ← Miro-ready AI tool calling flowcharts
...
- [MICROSERVICES-REFERENCE.md](architecture/service-boundaries.md)
- [services/README.md](services/README.md)
- [database/README.md](database/README.md)
- [MICROSERVICES-REVIEW.md](architecture/reviews/2026-04-microservices-review.md)
- [AI-SYSTEM-CONTEXT.md](architecture/ai-system-context.md)
- [features/ai-assistant.md](features/ai-assistant.md) — AI assistant feature doc (includes full pipeline reference)
- [features/tool-registry.md](features/tool-registry.md) — Tool registry feature doc (includes all 24 tools)
- [SRS.md](architecture/srs.md)
- [TESTING-GUIDE.md](development/testing.md)
...
- `/api/v1/workspaces/*/github-accounts*` -> integrations
- `/api/v1/workspaces/*/commits*` -> integrations
- `/api/v1/workspaces/*/git/*` -> integrations
- `/api/v1/git/webhook` -> integrations
- all other `/api/*` -> workspace

### Workspace Service

`services/workspace/` is the primary business service (modular monolith with DDD domains).

It owns:

- authentication routes (`domains/auth/`)
- workspace CRUD and membership management (`domains/workspace/`)
- invites and member skills (`domains/workspace/`)
- project CRUD and project membership (`domains/project/`)
- task CRUD and task daily reports (`domains/task/`)
- OPPM objectives, timeline, and costs (`domains/oppm/`)
- notifications (`domains/notification/`)
- dashboard stats (`domains/dashboard/`)
- agile workflows (`domains/agile/`)
- waterfall workflows (`domains/waterfall/`)
- Alembic migrations

### Intelligence Service

`services/intelligence/` owns AI-facing functionality.

It provides:

- workspace and project chat owned directly by the intelligence service routers (`domains/chat/`)
...
- RAG query endpoint (`domains/rag/`)
- internal commit analysis endpoint called by the integrations service (`domains/analysis/`)
- user feedback endpoint (rating + comment logged to `audit_log`)

The intelligence service is not advisory-only. Its tool handlers can create and update shared business data directly through AI-side repositories on the shared database, including projects, objectives, tasks, timeline entries, costs, risks, and deliverables.

See [features/ai-assistant.md](features/ai-assistant.md) and [features/tool-registry.md](features/tool-registry.md) for component-level detail.

### Integrations Service

`services/integrations/` owns GitHub integration.
...
- background handoff to the intelligence service for commit analysis

### Automation Service

`services/automation/` exposes Model Context Protocol tools through HTTP.
...
- migrations live in `services/workspace/alembic/`
- 29 tables across 7 domain groups (see [database/schema.md](database/schema.md) and [database/er-diagram.md](database/er-diagram.md))
...
| Semantic cache | `services/intelligence/infrastructure/rag/semantic_cache.py` | Embedding similarity cache for RAG results. Cosine threshold ≥ 0.92, TTL 300 s. Keys prefixed `ai:sem_cache:`. Fail-safe — returns `None` if Redis is unavailable. |
| Workspace membership cache | `shared/auth.py` | Two-tier cache (L1 in-process + L2 Redis) for workspace member roles. TTL 60 s. Keys prefixed `auth:membership:`. Invalidated on role change or member removal. Fail-safe — falls back to DB if Redis unavailable. |
...
```text
GitHub push webhook
  -> integrations service webhook endpoint
...
Frontend or AI flow
  ...
  -> intelligence service retrievers + reranker + memory loader
...
`HealthyRoundRobin` in `services/gateway/infrastructure/load_balancer.py` uses Python's `itertools.cycle` to
...
- workspace on `8000`
- intelligence on `8001`
- integrations on `8002`
- automation on `8003`
- mcp on `8003`
...
- migrations go through the workspace service migration flow
