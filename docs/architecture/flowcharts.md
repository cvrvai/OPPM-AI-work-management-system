...
These flowcharts describe runtime paths based on the current implementation. This is the **technical reference** — for Miro-ready presentation versions, see [miro/flowcharts.md](miro/flowcharts.md).
...
flowchart LR
    ...
    Gateway -->|/api/auth + default /api/v1| Core[Workspace Service]
    Gateway -->|/api/v1/workspaces/:ws/ai*| AI[Intelligence Service]
    Gateway -->|/api/v1/workspaces/:ws/rag*| AI
    Gateway -->|/api/v1/workspaces/:ws/projects/:id/ai*| AI
    Gateway -->|/api/v1/workspaces/:ws/github-accounts*| Git[Integrations Service]
    Gateway -->|/api/v1/workspaces/:ws/commits*| Git
    Gateway -->|/api/v1/workspaces/:ws/git/*| Git
    Gateway -->|/api/v1/workspaces/:ws/mcp/*| MCP[Automation Service]
...
flowchart TD
    ...
    B --> C[Store commits in integrations service]
...
flowchart TD
    ...
    H --> I[Workspace service validates token and membership rules]
...
flowchart TD
    ...
    B --> C[Integrations service locates repo_config by repository full name]
    ...
    H --> I[Integrations service calls intelligence /internal/analyze-commits]
    I --> J[Intelligence service analyzes commits against project context]
...
flowchart TD
    ...
    B -- Yes --> C[Forward to intelligence service]
    B -- No --> D{Path matches integrations pattern?}
    D -- Yes --> E[Forward to integrations service]
    D -- No --> F{Path matches automation pattern?}
    F -- Yes --> G[Forward to automation service]
    F -- No --> H[Forward to workspace service]
```
...
- The exact-match route `/internal/analyze-commits` is forwarded to the intelligence service for service-to-service use.
...
```mermaid
flowchart TD
    A[Intelligence service calls get_registry] --> B{Registry initialized?}
...
## 14. Workspace Service Function Flow (Workspace To OPPM)

```mermaid
flowchart TD
    A[Workspace Router /api/v1/workspaces/... ] --> B[shared.auth dependencies]
    B --> C{Role gate}
    C -- viewer/member/admin/owner --> D[Workspace service function]
...
## 15. Intelligence Service Function Flow (Chat And Tools)
...
## 16. Integrations Service Function Flow (Webhook To Analysis)
...
## 17. Automation Service Function Flow (Tool Discovery And Call)
