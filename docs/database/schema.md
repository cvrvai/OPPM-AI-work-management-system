...
The schema is defined by SQLAlchemy ORM models in `shared/models/` and managed through Alembic migrations in `services/workspace/alembic/`. There are currently **32 tables** across 10 domains.
...
| OPPM Templates | `oppm_templates`, `oppm_header`, `oppm_task_items` | 3 |
...
| Agile | `epics`, `user_stories`, `sprints`, `retrospectives` | 4 |
| Waterfall | `project_phases`, `phase_documents` | 2 |
| **Total** | | **32** |
...
OPPM spreadsheet template data per project (JSONB sheet data).

| Column | Type | Nullable | Default | Notes |
|---|---|---|---|---|
| `id` | `UUID` | NO | `uuid4()` | PK |
| `project_id` | `UUID` | NO | | FK → `projects(id)` CASCADE; UNIQUE |
| `workspace_id` | `UUID` | NO | | FK → `workspaces(id)` CASCADE |
| `sheet_data` | `JSONB` | NO | | OPPM spreadsheet content |
...
Per-project OPPM form header fields (free-text cells not on the projects table).
...
| `project_id` | `UUID` | NO | | FK → `projects(id)` CASCADE; UNIQUE |
| `workspace_id` | `UUID` | NO | | FK → `workspaces(id)` CASCADE |
| `project_leader_text` | `VARCHAR(200)` | YES | | "Project Leader:" free text |
| `completed_by_text` | `VARCHAR(200)` | YES | | "Project Completed By:" text |
| `people_count` | `INTEGER` | YES | | "# People working on the project" |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
...
Numbered major-task / sub-task rows in the OPPM template (distinct from general tasks table).
...
| `project_id` | `UUID` | NO | | FK → `projects(id)` CASCADE |
| `workspace_id` | `UUID` | NO | | FK → `workspaces(id)` CASCADE |
| `parent_id` | `UUID` | YES | | FK → `oppm_task_items(id)` CASCADE; NULL = major task |
| `task_id` | `UUID` | YES | | FK → `tasks(id)` SET NULL; optional link to real task |
| `number_label` | `VARCHAR(10)` | NO | | "1", "1.1", "2", etc. |
| `title` | `VARCHAR(500)` | NO | | Task title |
| `deadline_text` | `VARCHAR(100)` | YES | | Free-text deadline |
| `sort_order` | `INTEGER` | NO | `0` | Display order |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Auto-updates |
...
High-level feature buckets for agile project management.
...
| `title` | `VARCHAR(200)` | NO | | |
| `description` | `TEXT` | YES | `""` | |
| `status` | `VARCHAR(20)` | NO | `'open'` | CHECK: `open`, `in_progress`, `done` |
| `priority` | `VARCHAR(10)` | NO | `'medium'` | CHECK: `low`, `medium`, `high`, `critical` |
...
**Constraints:**
- `ck_epics_status` — status IN ('open', 'in_progress', 'done')
- `ck_epics_priority` — priority IN ('low', 'medium', 'high', 'critical')

**Indexes:** `workspace_id`, `project_id`

---

### `user_stories`

User stories linked to epics and sprints.
...
| `sprint_id` | `UUID` | YES | | FK → `sprints(id)` SET NULL |
...
| `priority` | `VARCHAR(10)` | NO | `'medium'` | CHECK: `low`, `medium`, `high`, `critical` |
| `status` | `VARCHAR(20)` | NO | `'draft'` | CHECK: `draft`, `ready`, `in_progress`, `done`, `rejected` |
| `position` | `INTEGER` | NO | `0` | Display order |
| `created_by` | `UUID` | YES | | FK → `workspace_members(id)` SET NULL |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Auto-updates |

**Constraints:**
- `ck_user_stories_status` — status IN ('draft', 'ready', 'in_progress', 'done', 'rejected')
- `ck_user_stories_priority` — priority IN ('low', 'medium', 'high', 'critical')
- `ck_user_stories_points` — story_points IS NULL OR story_points >= 0

**Indexes:** `workspace_id`, `project_id`, `epic_id`, `sprint_id`

---

### `sprints`

Time-boxed iterations for agile projects.
...
| `start_date` | `DATE` | YES | | |
| `end_date` | `DATE` | YES | | |
| `status` | `VARCHAR(20)` | NO | `'planning'` | CHECK: `planning`, `active`, `completed`, `cancelled` |
| `velocity` | `INTEGER` | YES | | |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Auto-updates |

**Constraints:**
- `ck_sprints_status` — status IN ('planning', 'active', 'completed', 'cancelled')
- `ck_sprints_velocity` — velocity IS NULL OR velocity >= 0
...
Sprint retrospective feedback (what went well, to improve, action items).
...
| `workspace_id` | `UUID` | NO | | FK → `workspaces(id)` CASCADE |
| `project_id` | `UUID` | NO | | FK → `projects(id)` CASCADE |
| `sprint_id` | `UUID` | NO | | FK → `sprints(id)` CASCADE; UNIQUE |
| `went_well` | `JSONB` | YES | `[]` | List of positive items |
| `to_improve` | `JSONB` | YES | `[]` | List of improvement items |
| `action_items` | `JSONB` | YES | `[]` | List of action items |
| `created_by` | `UUID` | YES | | FK → `workspace_members(id)` SET NULL |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Auto-updates |

**Indexes:** `workspace_id`, `project_id`, `sprint_id`
...
Waterfall methodology phases per project.
...
| `workspace_id` | `UUID` | NO | | FK → `workspaces(id)` CASCADE |
| `project_id` | `UUID` | NO | | FK → `projects(id)` CASCADE |
| `phase_type` | `VARCHAR(30)` | NO | | CHECK: `requirements`, `design`, `development`, `testing`, `deployment`, `maintenance` |
| `status` | `VARCHAR(20)` | NO | `'not_started'` | CHECK: `not_started`, `in_progress`, `completed`, `approved` |
| `position` | `INTEGER` | NO | | Display order |
| `start_date` | `DATE` | YES | | |
| `end_date` | `DATE` | YES | | |
| `gate_approved_by` | `UUID` | YES | | FK → `workspace_members(id)` SET NULL |
| `gate_approved_at` | `TIMESTAMPTZ` | YES | | |
| `notes` | `TEXT` | YES | | |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Auto-updates |

**Constraints:**
- `ck_project_phases_type` — phase_type IN ('requirements', 'design', 'development', 'testing', 'deployment', 'maintenance')
- `ck_project_phases_status` — status IN ('not_started', 'in_progress', 'completed', 'approved')
- `uq_project_phases_project_type` — UNIQUE(`project_id`, `phase_type`)
...
Documents attached to waterfall phases.
...
| `title` | `VARCHAR(300)` | NO | | |
| `content` | `TEXT` | YES | `""` | |
| `document_type` | `VARCHAR(50)` | NO | `'general'` | |
| `version` | `INTEGER` | NO | `1` | CHECK: >= 1 |
| `created_by` | `UUID` | YES | | FK → `workspace_members(id)` SET NULL |
| `created_at` | `TIMESTAMPTZ` | NO | `now()` | |
| `updated_at` | `TIMESTAMPTZ` | NO | `now()` | Auto-updates |

**Constraints:** `ck_phase_documents_version` — version >= 1

**Indexes:** `workspace_id`, `project_id`, `phase_id`
...
projects.id
  ...
  ← oppm_templates.project_id
  ← oppm_header.project_id
  ← oppm_task_items.project_id
  ← repo_configs.project_id
  ← epics.project_id
  ← user_stories.project_id
  ← sprints.project_id
  ← retrospectives.project_id
  ← project_phases.project_id
  ← phase_documents.project_id
...
tasks.id
  ...
  ← oppm_task_items.task_id
  ← commit_analyses.matched_task_id
  ← user_stories.task_id

epics.id
  ← user_stories.epic_id

sprints.id
  ← user_stories.sprint_id
  ← retrospectives.sprint_id

project_phases.id
  ← phase_documents.phase_id
...
oppm_task_items.id
  ← oppm_task_items.parent_id (self-ref)
...
Migrations are managed by Alembic in `services/workspace/alembic/versions/`.
...
6. Keep migrations in `services/workspace/alembic/versions/`
7. Update this document and [ER-DIAGRAM.md](ER-DIAGRAM.md) with every schema change
