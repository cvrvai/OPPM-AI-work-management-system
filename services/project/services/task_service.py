...
Migrated from services/core/services/task_service.py.
"""
...
async def create_task(
    session: AsyncSession, data: dict, workspace_id: str, user_id: str, member_id: str | None = None
) -> dict:
    task_repo = TaskRepository(session)
    project_repo = ProjectRepository(session)
...
async def update_task(
    session: AsyncSession, task_id: str, data: dict, workspace_id: str, user_id: str
) -> dict:
    task_repo = TaskRepository(session)
    audit_repo = AuditRepository(session)
...
async def delete_task(
    session: AsyncSession, task_id: str, workspace_id: str, user_id: str
) -> None:
    task_repo = TaskRepository(session)
    audit_repo = AuditRepository(session)
...
async def list_task_reports(session: AsyncSession, task_id: str) -> list:
...
async def create_task_report(
    session: AsyncSession, task_id: str, user_id: str, member_id: str | None, data: dict
) -> dict:
    report_repo = TaskReportRepository(session)
...
async def approve_task_report(
    session: AsyncSession, report_id: str, user_id: str, data: dict
) -> dict:
...
async def delete_task_report(
    session: AsyncSession, report_id: str, user_id: str
) -> None:
    report_repo = TaskReportRepository(session)
    report = await report_repo.find_by_id(report_id)
    if not report:
        raise HTTPException(status_code=404, detail="Report not found")
    if str(report.reporter_id) != user_id:
