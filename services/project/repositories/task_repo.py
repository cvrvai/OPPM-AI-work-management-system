"""Task repository — migrated from services/core/repositories/task_repo.py."""
...
class TaskRepository(BaseRepository):
    ...
    async def get_project_progress_data(self, project_id: str) -> list[dict]:
        stmt = select(Task.progress, Task.project_contribution).where(Task.project_id == project_id)
        result = await self.session.execute(stmt)
    ...
    async def set_dependencies(self, task_id: str, depends_on: list[str]) -> None:
        await self.session.execute(delete(TaskDependency).where(TaskDependency.task_id == task_id))
        if depends_on:
