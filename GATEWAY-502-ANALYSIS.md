...
```bash
# Direct call to Core service (inside Docker)
...
```bash
# Core service can reach other services
docker exec oppmaiworkmanagementsystem-core-1 \
...
export default defineConfig({
  server: {
    proxy: {
      '/api': {
        target: 'http://localhost:8000',  // Core service
        changeOrigin: true,
...
Frontend (localhost:5173)
  → /api/* request
    → Vite proxy intercepts
      → Forwards to localhost:8000 (Core service)
