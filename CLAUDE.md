...
## Key Commands
- **Backend (core)**: `cd apps/core && uvicorn main:app --reload --port 8000`
- **Frontend**: `cd frontend && npm run dev`
- **Type check**: `cd frontend && npx tsc -b`
- **Build**: `cd frontend && npm run build`

## Important Notes
- Backend uses direct DB owner role (no RLS). JWT validated locally via `python-jose` in `packages/shared/auth.py`.
