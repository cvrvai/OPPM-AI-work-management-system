...
### Day 3: Know the Code
- [ ] Review `services/core/routers/v1/` (API design)
- [ ] Review `services/core/services/` (business logic)
- [ ] Review `services/core/repositories/` (data access)
...
**Say this:**
> "This is a multi-tenant SaaS platform with four microservices. Frontend is React 19 with TypeScript. Backend is Python FastAPI using 4-layer clean architecture. We have a Core service for auth and projects, AI service for LLM capabilities, Git service for GitHub integration, and MCP service for protocol support. All services communicate through an Nginx gateway over a Docker bridge network. Data is in PostgreSQL, caching in Redis. Why this design? Independent scaling, clear separation of concerns, testability."

### Part 2: Code Quality (4 min)
**Show these files:**
1. `services/core/routers/v1/projects.py` - API endpoint
2. `services/core/services/project_service.py` - Business logic
3. `services/core/repositories/project_repo.py` - Data access
...
2. `services/core/middleware/` - Auth middleware
3. `services/core/schemas/` - Input validation
